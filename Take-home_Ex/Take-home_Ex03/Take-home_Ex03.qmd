...
```{r}
boxplot(company$revenue)
```

```{r}
quartiles <-quantile(company$revenue, probs=c(.25,.75), na.rm=TRUE)
IQR<-IQR(company$revenue)
...
boxplot(company2$revenue)
```

```{r}
quartiles <-quantile(company2$revenue, probs=c(.25,.75), na.rm=TRUE)
IQR<-IQR(company2$revenue)
...
We performed statistical analysis to find out if the mean revenue is different for the different industries. From the results, we can see that fishing and service industries tend to have lower mean revenue than food industry.
