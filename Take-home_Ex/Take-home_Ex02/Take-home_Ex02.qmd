...
We calculated the weight by using frequency count of the number of links by *source, target and monthyear*. We also calculate the average *weightkg* for the aggregated data.
...
::: callout-note
The median weight is 1 while the mean weight is 26, indicating very left skewed distribution of shipments. We are only interested in shipments with unusual higher frequency. We can choose to look at only *source* nodes that have shipped the highest number of shipments and *target* nodes that have received the highest number of shipments
:::

## Find top sources and targets
...
```{r}
top_MC2_graph<-tbl_graph(nodes=top_MC2_nodes,
                         edges=top_MC2_edges,
                         directed = TRUE)
```
